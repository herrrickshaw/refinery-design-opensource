...
* `companies/` - a data layer, not a process model: `core.py` (series lookup, derived utilisation with a >125% flag that checks for a company-reported confirmation, roll-ups that sum only refineries with both throughput and capacity, `then_vs_now` with a commissioning guard, FCC summary that counts revamps separately), one module per company for its caveats, and `scripts/build_companies_data.py` (mechanical normalisation of the research files plus a hand-transcribed FCC unit table and change timeline).
